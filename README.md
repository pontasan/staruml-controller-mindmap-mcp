# StarUML Controller MCP Server — Mind Map Diagram

An MCP (Model Context Protocol) server specialized for **Mind Map Diagrams**. Enables AI assistants like Claude to programmatically create and manage mind map diagrams — nodes and edges in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="AI Technology Overview — Mind Map created with staruml-controller-mindmap-mcp" width="700">
</p>

<p align="center"><em>Example: AI Technology Overview mind map — fully generated via MCP tools</em></p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-mindmap-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-mindmap-mcp.git
cd staruml-controller-mindmap-mcp
npm install && npm run build
```
...
5. All Mind Map Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-mindmap": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-mindmap-mcp/dist/index.js"],
...
```bash
claude mcp add staruml-mindmap -e STARUML_PASSWORD=<password> -- node /absolute/path/to/staruml-controller-mindmap-mcp/dist/index.js
```
...
The core of this server — full CRUD for every Mind Map Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `mindmap_list_diagrams`, `mindmap_create_diagram`, `mindmap_get_diagram`, `mindmap_update_diagram`, `mindmap_delete_diagram` |
| **Nodes** | `mindmap_list_nodes`, `mindmap_create_node`, `mindmap_get_node`, `mindmap_update_node`, `mindmap_delete_node` |
| **Edges** | `mindmap_list_edges`, `mindmap_create_edge`, `mindmap_get_edge`, `mindmap_update_edge`, `mindmap_delete_edge` |
